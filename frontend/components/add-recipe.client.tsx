          ...
          <div className="flex flex-col mt-4 p-4 border rounded-md">
            ...
            {filePreviews.map((preview, index) => (
              <div key={index} className="mt-4 relative flex items-center justify-center bg-secondary rounded-md overflow-hidden w-full">
                ...
                <Button variant="secondary" size="icon" className="absolute top-2 right-2" onClick={() => handleRemoveFile(index)}>
                  <X />
                </Button>
              </div>
            ))}
            <div className="pt-4 flex justify-center gap-4">
              {filePreviews.length < 5 && <ImageUploader onImagesUpload={handleImagesUpload} />}
              {filesData.length !== 0 && (
                <Button variant="default" onClick={submitImagesToApi} disabled={isUploadingFiles}>
                  {isUploadingFiles ? <Loader2 className="mr-2 h-5 w-5 animate-spin" /> : "Submit"}
                </Button>
              )}
            </div>
          </div>
            ...
            {recipeText.trim().length > 0 && (
              <div className="pt-4 mx-auto">
                <Button variant="default" onClick={submitTextToApi} disabled={isSubmittingText}>
                  {isSubmittingText ? <Loader2 className="mr-2 h-5 w-5 animate-spin" /> : "Submit"}
                </Button>
              </div>
            )}
          </div>
